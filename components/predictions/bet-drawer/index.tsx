  ...
  return (
    <Drawer open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DrawerContent className="rounded-t-3xl bg-background! shadow-lg! backdrop-blur-2xl! outline-none p-0! border-t border-border/50! flex flex-col max-h-[92dvh]!">
        ...
        <div className="flex flex-col px-4 md:px-5 pt-4 md:pt-5 pb-0 space-y-4 md:space-y-5 overflow-y-auto">
          {/* Top Row: Buy dropdown + Market order */}
          <div className="flex items-center justify-between">
            <p className="text-sm font-semibold text-foreground">Place Bet</p>
            <Button
              onClick={onClose}
              size="icon"
              className="rounded-full! bg-secondary! text-foreground! hover:bg-secondary! hover:text-foreground! p-2! h-auto! w-auto!"
            >
          ...
          <div className="flex items-center gap-3.5">
            <div className="relative w-11 h-11 md:w-12 md:h-12 rounded-xl overflow-hidden shrink-0 bg-card">
              <Image
          ...
          <div className="flex items-center justify-between gap-2 md:gap-4 py-2 md:py-4">
            <Button
              ...
              className="w-10 h-10 md:w-11 md:h-11 rounded-full flex items-center justify-center text-muted-foreground! hover:bg-secondary! active:bg-secondary! active:text-foreground! transition-all bg-transparent! active:scale-105!"
            >
              <Minus className="w-5 h-5" />
            </Button>

            <div className="flex-1 flex items-center justify-center relative h-14 md:h-16 w-full">
              <input
                type="text"
                inputMode="decimal"
                ...
                className="w-full text-center bg-transparent outline-none border-none p-0 focus:ring-0 text-3xl md:text-4xl font-bold text-foreground tabular-nums placeholder:text-muted-foreground/50 font-mono!"
          ...
          <div className="grid grid-cols-5 gap-2">
            {PRESET_AMOUNTS.map((preset) => (
              <Button
                ...
                className="p-1.5 min-h-9 rounded-lg bg-secondary! text-foreground! text-xs font-semibold active:scale-95 transition-transform hover:bg-secondary!/80"
                size="sm"
              >
            ...
            <Button
              onClick={() => setAmount("0")}
              className="p-1.5 min-h-9 rounded-lg bg-secondary! text-foreground! text-xs font-bold active:scale-95 transition-transform hover:bg-secondary!/80"
              size="sm"
            >
              Max
            </Button>
          ...
          <div className="flex flex-col gap-3 px-1 md:px-2 py-3 mt-1 h-auto! border-t border-border/50!">
            <div className="flex items-center justify-between ">
            ...
            <div className="flex items-center justify-between gap-2">
              ...
              <p className="text-success text-xl font-mono">
                {toWin.amount.toLocaleString("en-US", {
              ...
              {showDetails && (
                <div className="flex flex-col gap-2.5 mt-3.5 pt-3.5 text-sm border-t border-border/50!">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <p className="text-success text-sm">+{roi.toFixed(1)}%</p>
                  </div>
                  <div className="flex items-center justify-between">
                    <p className="text-muted-foreground font-medium">
                      Price impact
                    </p>
                    <p className="text-success text-sm">&lt; 1%</p>
                  </div>
                  <div className="flex items-center justify-between">
                    <p className="text-muted-foreground font-medium">
                      Trading fee
                    </p>
                    <p className="text-success text-sm">Free</p>
                  </div>
                  ...
                  <div className="mt-4 flex items-start gap-1.5 p-2 bg-secondary/30 rounded-lg">
                    ...
                    <p className="text-muted-foreground font-medium text-sm">
                      Orders are matched via AMM. Final execution price and
                      shares may vary slightly due to market movements.
                    </p>
                  </div>
                </div>
              )}
          ...
          {/* Deposit Button */}
          <div className="w-full sticky bottom-0 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/70 pb-[calc(0.75rem+env(safe-area-inset-bottom))] pt-2">
            <Button
              onClick={() => {
              ...
              size="lg"
              className="rounded-xl! font-semibold! bg-[#16a34a]! text-accent-foreground! w-full! active:scale-95! transition-transform! min-h-11"
            >
              Confirm Bet
            </Button>
          </div>
